...
def move_start_final(w):
    ...
    if w != 2:
        start_to_final -= 1
    left_water -= water_consume[w]*2
    left_food -= food_consume[w]*2
...
def buy_something_need(n):
    ...
    fund_count = (water_need*water_price+food_need*food_price)*2
...
def plan_1_todo(w_list):
    ...
    策略1：
    八次移动直接前往终点
    至少准备8天高温，3天沙暴的物资，中途可以购买补给
    前五天遇到n次沙暴就补抵御n-1次沙暴的物资
    ...
    start_to_final = 8
    today = 1
    left_water = count_bad_moment(8, 3, 0)[0]
    left_food = count_bad_moment(8, 3, 0)[1]
    left_fund = count_start_fund(8, 3, 0)
    if w_list[:11].count(2) < 4:
        while start_to_final > 0:
            move_start_final(w_list[today-1])
            today += 1
    elif w_list[:5].count(2) > 0:
        buy_something_need(w_list[:5].count(2)-1)
        while start_to_final > 0:
            move_start_final(w_list[today-1])
            today += 1
